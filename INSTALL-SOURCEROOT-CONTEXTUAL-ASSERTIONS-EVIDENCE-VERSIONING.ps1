...
$StageFolderName = "SourceRoot-Contextual-Assertions-Evidence-Versioning-v1"
$NormalizedStageName = "sourceroot-contextual-assertions-evidence-versioning-v1"
...
if ($MissingMarkers.Count -gt 0) {
    Stop-Install "Repository or Chunk 0-3 markers are missing: $($MissingMarkers -join ', ')"
}
...
if (
    $Manifest.stageName -ne "SourceRoot-Contextual-Assertions-Evidence-Versioning" -or
    $Manifest.stageVersion -ne "v1" -or
    $Manifest.targetRepository -ne "dictionaryhub" -or
    $Manifest.requiredPreviousStage -ne "SourceRoot Chunk 3 - Contextual Identity and Time Refinement v1"
) {
...
foreach ($RelativePathValue in $AllFiles) {
    ...
    $ResolvedSource = [IO.Path]::GetFullPath((Join-Path $PayloadRoot ($RelativePath -replace '/', '\')))
    $ResolvedPayloadRoot = [IO.Path]::GetFullPath($PayloadRoot).TrimEnd("\", "/")
    if (-not $ResolvedSource.StartsWith($ResolvedPayloadRoot + "\", [StringComparison]::OrdinalIgnoreCase)) {
...
Write-Host "SourceRoot Contextual Assertions, Evidence, and Versioning v1 installation report" -ForegroundColor Cyan
...
Write-Host "[PASS] SourceRoot Contextual Assertions, Evidence, and Versioning v1 installed and verified." -ForegroundColor Green
