...
$StageFolderName = "SourceRoot-Registry-API-Contract-v1"
$NormalizedStageName = "sourceroot-registry-api-contract-v1"
...
if ($MissingMarkers.Count -gt 0) {
    Fail-Installation "Repository or Chunk 0 markers are missing: $($MissingMarkers -join ', ')"
}
...
if (
    $Manifest.stageName -ne "SourceRoot-Registry-API-Contract" -or
    $Manifest.stageVersion -ne "v1" -or
    $Manifest.targetRepository -ne "dictionaryhub"
) {
    Fail-Installation "Package manifest identity does not match SourceRoot Registry API Contract v1."
}
...
Write-Host ""
Write-Host "SourceRoot Registry API Contract v1 installation report" -ForegroundColor Cyan
...
$VerifierPath = Join-Path $RepositoryRoot "VERIFY-SOURCEROOT-REGISTRY-API-CONTRACT.ps1"
...
Write-Host ""
Write-Host "[PASS] SourceRoot Registry API Contract v1 installed and verified." -ForegroundColor Green
