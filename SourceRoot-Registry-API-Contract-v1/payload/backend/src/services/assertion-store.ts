...
export interface ListAssertionsOptions {
  ...
  limit: number;
  offset?: number;
  sort?: "label" | "createdAt" | "updatedAt" | "assertionId";
...
export async function listAssertions(
  ...
  if (options.sourceId) {
    filterValues.push(options.sourceId);
    conditions.push(`
      EXISTS (
        SELECT 1
        FROM assertion_sources filter_source
        WHERE filter_source.assertion_id = a.assertion_id
          AND filter_source.source_id = $${filterValues.length}
      )
    `);
  }
