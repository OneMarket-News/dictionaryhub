...
async function verifyPage(client, sessionId, baseUrl, pageName, viewport) {
  ...
  const result = await client.send("Runtime.evaluate", {
    ...
    expression: `(() => {
      ...
      return {
        ...
        accountSurface: Boolean(document.querySelector('#dictionaryrootAccountStatusTitle')),
        workflowSurface: Boolean(document.querySelector('#dictionaryrootWorkflowTitle')),
        adminSurface: Boolean(document.querySelector('#dictionaryrootAdminIdentity')),
        href: location.href,
  ...
  if (pageName === "account-v1.html" && !value.accountSurface) failures.push("account surface is missing");
  if (pageName === "workflow-v1.html" && !value.workflowSurface) failures.push("workflow surface is missing");
  if (pageName === "admin-v1.html" && !value.adminSurface) failures.push("administration surface is missing");
